...
#   geom_bar(stat = "identity",
#            position=position_dodge(width=0.9))+
#   geom_text(aes(label = paste0(round(perc, 0), "%"), y=perc-2), size = 4.5,
#             position = position_dodge(width=0.9)
#             )+
#   
#   scale_fill_brewer(palette = "Spectral")+
...
#            position=position_dodge(width=0.9))+
#   geom_text(aes(label = paste0(round(perc, 0), "%"), y=perc-2), size = 4.5,
#             position = position_dodge(width=0.9), color= "grey27"
#             )+
#   
#   scale_fill_brewer(palette = "Spectral")+
#   ylim(0, 100)+
...
cart3 <- cart2 %>% 
  ...
  mutate(
         CRS_grade2 = case_when(
           `Max CRS grade (0-5)` < 2                              ~ "No CRS or Grade < 2",
           `Max CRS grade (0-5)` >= 2                             ~ "Grade ≥2"),
         CRS_grade2 = factor(CRS_grade2, levels = c("No CRS or Grade < 2", "Grade ≥2"))
  )

# glm(gr3_cytopenia_30d ~ `High Marrow Burden? (yes=1, no=0)` + `gr3_cytopenia_-5d` + CRS_grade2 + `Steroid use (Yes=1, No=0)` + `Number of prior lines of therapy`,
#     data = cart3,
#     family = "binomial") %>%
...
#   bold_p(t = .05) 
# 
# glm(gr3_cytopenia_90d ~ `High Marrow Burden? (yes=1, no=0)` + `gr3_cytopenia_-5d` + CRS_grade2 + `Steroid use (Yes=1, No=0)` + `Number of prior lines of therapy`,
#     data = cart3,
#     family = "binomial") %>%
#   tbl_regression(exponentiate = TRUE, 
#                  intercept = TRUE) %>%
...
#   bold_p(t = .05) 
# 
# glm(gr3_cytopenia_30d ~ `High Marrow Burden? (yes=1, no=0)` + CRS_grade2 + `Steroid use (Yes=1, No=0)` + `Number of prior lines of therapy`,
#     data = cart3,
#     family = "binomial") %>%
#   tbl_regression(exponentiate = TRUE, 
#                  intercept = TRUE) %>%
...
forest_p <- 
  ...
  filter(!str_detect(Characteristic, "YES|NO|CRS_grade2|No CRS")) %>% 
  separate(`95% CI`, into = c("lower", "upper"), sep = ", ") %>% 
  mutate_at(c("OR", "lower", "upper"), ~ as.numeric(.)) %>% 
  mutate(name= c("High marrow burden (≥50%)",
                 "Grade ≥ 3 Cytopenia at Day -5",
                 "Grade ≥ 2 CRS",
                 "Steroid use",
                 "Number of prior lines of therapy"
                 ))
