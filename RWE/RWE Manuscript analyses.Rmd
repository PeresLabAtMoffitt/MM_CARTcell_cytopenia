...
MM_cart <-
  readxl::read_xlsx(paste0(path,
    "/RWE/FINAL 4.13.22 ASCO 2022 Myeloma CAR-T SOC Outcomes Spreadsheet.xlsx"),
                    skip = 1, n_max = 198
                    ) %>% 
...
cart0 %>% 
  ...
  tbl_summary(
    type = list(
      c(`Extramedullary Disease? (Yes, No)`, `High Marrow Burden (>= 50%)`,
    #     cyto, 
    `Deletion 17p prior to CAR-T infusion (yes, no)`,
        `t(4:14) prior to CAR-T infusion (yes, no)`,
        `t(14:16) prior to CAR-T infusion (yes, no)`,
...
tbl1 <- cart0 %>% 
  ...
  tbl_summary(type = list(
    c(
      `Day of Max ICANS (relative to infusion)`) ~ "continuous"),
    statistic=list(all_continuous() ~ "{median} ({min}, {max})"), 
    digits = all_continuous() ~ 1,
    missing = "no") %>% 
  bold_labels() %>% add_stat_label() %>% 
...
#   select(Age, agecat, male_sex, Sex,
#          `Extramedullary Disease? (Yes, No)`, 
#          `High Marrow Burden (>= 50%)`, `ECOG at LD`, `R-ISS at CAR-T infusion`, #cyto, 
#          `Bridging Therapy? (Yes, No)`,
#          `Prior auto SCT (yes, no)`, 
#          `Prior treatment with any gene therapy-based therapeutic or investigational cellular therapy or BCMA targeted therapy (Yes, No)`,
#          immuno, proteasome, dara, doubleref, tripleref, pentaref, 
...
#     type = list(
#     `Extramedullary Disease? (Yes, No)` ~ "categorical",
#     # cyto ~ "categorical",
#     `Bridging Therapy? (Yes, No)` ~ "categorical",
#     `Prior auto SCT (yes, no)` ~ "categorical",
...
#     `Did patient met criteria for KarMMa1 pre-CAR-T?` ~"categorical"),
#     
#     statistic = list(all_continuous() ~ "{median} ({min}, {max})"), 
#     digits = all_continuous() ~ 1
#     ) %>% 
...
# #   filter(is.na(Not_evaluable_at_30days)) %>% 
# #   filter(day30_response_group == "CR or sCR") %>% 
# #   select(MRDneg_30days, gr3_cytopenia_30d) %>% 
# #   tbl_summary(by =`gr3_cytopenia_30d`,
# #               type = list(c(MRDneg_30days) ~ "categorical"),
# #               label = list(MRDneg_30days ~ "MRDneg_30days within CR/sCR"),
# #               digits = all_continuous() ~ 1,
# #               missing = "no") %>% 
# #   bold_labels() %>% add_stat_label() %>% add_p()
...
# #   filter(is.na(Not_evaluable_at_30days)) %>% 
# #   filter(day90_response_group == "CR or sCR") %>% 
# #   select(MRDneg_90days, gr3_cytopenia_30d) %>% 
# #   tbl_summary(by =`gr3_cytopenia_30d`,
# #               type = list(c(MRDneg_90days) ~ "categorical"),
# #               label = list(MRDneg_90days ~ "MRDneg_90days within CR/sCR"),
# #               digits = all_continuous() ~ 1,
# #               missing = "no") %>% 
# #   bold_labels() %>% add_stat_label() %>% add_p()
...
cart0 %>% 
  ...
  select(CRS_gradecat, ICANS_gradecat,
         ...
         `Bridging Therapy? (Yes, No)`, any_infection) %>% 
...
cart0 %>% 
  ...
  select(CRS_gradecat, ICANS_gradecat,
         `Steroid use (Yes=1, No=0)`, 
         `Tocilizumab (Yes=1, No=0)`, `Anakinra (Yes=1, No=0)`,
         `Bridging Therapy? (Yes, No)`, any_sevinfection) %>% 
...
# # table(cart$CRS_fup2)
# # 
# # cart0$CRS_fup[cart0$CRS_fup==30] <- 7
# CRS <- 
#  cuminc(
#     ftime = cart0$`Latest Day of CRS (relateive to infusion)`, 
#     fstatus = cart0$CRS_any2, 
#     group = cart0$`Infection type`)
...
#     day == "apheresis"  ~ 0
#   )) %>% 
#   ggplot(aes(x= day, y= value, color= patient_id))+
#   geom_point()+
#   geom_line()+
#   scale_x_continuous(
#     breaks = c(0, 90),
...
#   filter(!is.na(value)) %>% 
#   arrange(value) %>% 
#   ggplot(aes(x= day, y= fct_reorder2(patient_id, value, day), fill= value))+
...
alluvial_A <- cart2 %>% 
  select(patient_id, igg_hsv_change,
         `IgG HSV1/2 apheresis`, `IgG HSV1/2 D90`) %>%
  # mutate(igg_hsv_change = case_when(
...
# Alluvial
alluvial_B <- cart2 %>% 
  select(patient_id, igg_vzv_change,
         `VZV IgG apheresis`, `VZV IgG D90 (1=pos, 0=neg)`
  ) %>% 
  # mutate(igg_vzv_change = case_when(
...
# Alluvial
alluvial_C <- cart2 %>% 
  select(patient_id, igg_cmv_change,
         `CMV IgG apheresis`, `CMV IgG D90 (1=pos, 0=neg)`
  ) %>% 
  # mutate(igg_cmv_change = case_when(
...
mat <- cart2 %>% 
  select(patient_id, intersect(starts_with("Pneumo"), ends_with("change"))) %>% 
  drop_na() %>% 
  # mutate(new_sort = rowSums(select(.,-patient_id), na.rm = TRUE)) %>%
  mutate(new_sort = 
           c(5,7,8,23,6,9,10,11,12,4,13,14,26,15,
             24,3,16,25,17,2,18,21,19,20,1,22)) %>%
...
#                             col = c("darkgreen", "pink", "orange", rep("blue", 28))),
