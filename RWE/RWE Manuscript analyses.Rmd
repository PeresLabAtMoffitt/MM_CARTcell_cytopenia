...
# CRS 
  mutate(CRS_any = ifelse(`Max CRS grade (0-5)` == 0, "NO", "YES"),
         ...
         gr2_CRS = case_when(
           `Max CRS grade (0-5)` %in% c(0:1)                      ~ "Grade <2",
           `Max CRS grade (0-5)` %in% c(2:3)                      ~ "Grade ≥2",
           TRUE                                                   ~ NA_character_
           )) %>% 
  # mutate( = case_when(
  #    == 30                ~ NA,
  ...
  mutate(ICANS_any = ifelse(`Max ICANS (relative to infusion)` == 0, "NO", "YES"),
         ...
         gr2_ICANS = case_when(
           ...
           `Max ICANS (relative to infusion)` %in% c(2:3)         ~ "Grade ≥2",
           TRUE                                                   ~ NA_character_
         )) %>% 
...
## From here, we excluded 38 patients
```{r}
cart0 %>% 
...
cart0 %>% 
  ...
  add_p() %>% 
  modify_footnote(update = everything() ~ NA) %>%
  modify_spanning_header(all_stat_cols() ~ "**Grade 2 CRS**")
```
...
cart1 <- cart0 %>% 
  mutate(day90_response_sCRvsOthers = case_when(
    day90_response_sCRvsOthers == "Others"       ~ 0,
    day90_response_sCRvsOthers == "CR or sCR"    ~ 1
  ), 
  day90_response_sCRvsOthers = factor(day90_response_sCRvsOthers)) %>% 
  mutate(ORR_90days = case_when(
    ORR_90days == "PD/SD/MR"    ~ 0,
    ORR_90days == "ORR"         ~ 1
  ), 
  ORR_90days = factor(ORR_90days))
```

### CR or better
```{r}
glm(day90_response_sCRvsOthers ~ `Prior treatment with any gene therapy-based therapeutic or investigational cellular therapy or BCMA targeted therapy (Yes, No)` + cytogenetics + `Extramedullary Disease? (Yes, No)` + cart_cell_dose +
        `ECOG at LD` + pentaref + number_prior_lines_therapy + Age,
                data = cart1,
    family = "binomial"(link='logit')) %>%
            tbl_regression(exponentiate = TRUE) %>%
            modify_spanning_header(everything() ~ "CR or sCR at 90 days vs Others")
```
...
```{r}
glm(ORR_90days ~ `Prior treatment with any gene therapy-based therapeutic or investigational cellular therapy or BCMA targeted therapy (Yes, No)` + cytogenetics + `Extramedullary Disease? (Yes, No)` + cart_cell_dose +
        `ECOG at LD` + pentaref + number_prior_lines_therapy + Age,
                data = cart1,
    family = "binomial") %>%
            tbl_regression(exponentiate = TRUE) %>%
...
tbl1 <- tbl_survfit(
  ...
  include = c(Overall, Did_patient_met_criteria_for_KarMMa1_pre_CAR_T#, 
              # CRS_any, CRS_gradecat, ICANS_any, ICANS_gradecat
              ),
  probs = 0.5,
