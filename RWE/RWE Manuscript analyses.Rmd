...
myplot <- ggsurvplot(survfit(Surv(days_at_pfs_from_infusion, pfs_event) ~ 1, 
                   data = cart0),
...
cart0 %>%
  ...
  mutate(`Cause of death (myeloma, toxicitiy,other)` = case_when(
    str_detect(`Cause of death (myeloma, toxicitiy,other)`, "Myeloma")      ~ "Myeloma",
    str_detect(`Cause of death (myeloma, toxicitiy,other)`, "COVID")        ~ "COVID-19",
    str_detect(`Cause of death (myeloma, toxicitiy,other)`, "oxicity")      ~ "Toxicity",
    TRUE                                     ~ `Cause of death (myeloma, toxicitiy,other)`
  )) %>% 
  select(`Cause of death (myeloma, toxicitiy,other)`) %>%
  tbl_summary() %>%
  bold_labels() %>% add_stat_label()
```
...
tbl4 <- cart0 %>%
  ...
  cohort = factor(cohort, levels = c("Overall",
                                     "Met criteria for KarMMa1 pre-CAR-T",
                                     "NOT et criteria for KarMMa1 pre-CAR-T"))) %>% 
  select(CRS_gradecat, cohort) %>%
  tbl_summary(by= CRS_gradecat,
              # type = list(c(Did_patient_met_criteria_for_KarMMa1_pre_CAR_T) ~ "categorical"),
              statistic = all_categorical() ~ "{p}%",
              percent = "row") %>%
  bold_labels() %>% add_stat_label() %>% 
...
tbl6 <- cart0 %>%
  ...
  cohort = factor(cohort, levels = c("Overall",
                                     "Met criteria for KarMMa1 pre-CAR-T",
                                     "NOT et criteria for KarMMa1 pre-CAR-T"))) %>% 
  select(ICANS_gradecat, cohort) %>%
  tbl_summary(by= ICANS_gradecat,
              # type = list(c(Did_patient_met_criteria_for_KarMMa1_pre_CAR_T) ~ "categorical"),
              statistic = all_categorical() ~ "{p}%",
              percent = "row") %>%
  bold_labels() %>% add_stat_label() %>% 
