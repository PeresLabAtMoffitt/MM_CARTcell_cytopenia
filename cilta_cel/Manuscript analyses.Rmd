...
day7 <- cilta %>% 
  ...
  filter(pfs_event == 0 | pfs_from_infusion_months > (7 / 30.417)) %>% 
  select(pt_identifier) %>% 
  tbl_summary(missing = "no") %>% 
  bold_labels() %>% add_stat_label()
day14 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  filter(pfs_event == 0 | pfs_from_infusion_months > (14 / 30.417)) %>% 
  select(pt_identifier) %>% 
  tbl_summary(missing = "no") %>% 
  bold_labels() %>% add_stat_label()
day21 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  filter(pfs_event == 0 | pfs_from_infusion_months > (21 / 30.417)) %>% 
  select(pt_identifier) %>% 
  tbl_summary(missing = "no") %>% 
  bold_labels() %>% add_stat_label()
day30 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  filter(pfs_event == 0 | pfs_from_infusion_months > (30 / 30.417)) %>% 
  select(pt_identifier) %>% 
  tbl_summary(missing = "no") %>% 
  bold_labels() %>% add_stat_label()
day90 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  filter(pfs_event == 0 | pfs_from_infusion_months > 3) %>% 
  select(pt_identifier) %>% 
  tbl_summary(missing = "no") %>% 
  bold_labels() %>% add_stat_label()
day180 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  filter(pfs_event == 0 | pfs_from_infusion_months > 6) %>% 
  select(pt_identifier) %>% 
  tbl_summary(missing = "no") %>% 
  bold_labels() %>% add_stat_label()
mon12 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  filter(pfs_event == 0 | pfs_from_infusion_months > 12) %>% 
  select(pt_identifier) %>% 
  tbl_summary(missing = "no") %>% 
  bold_labels() %>% add_stat_label()
tbl_merge(list(day7, day14, day21, day30, day90, day180, mon12), 
          tab_spanner = c("day7", "day14", "day21", "day30", "day90", "day180", "month12"))
```
...
cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  filter(os_event == 1) %>% 
  select(`cause of death` = cause_of_death_myeloma_toxicitiy_unrelated) %>% 
  tbl_summary() %>% 
  bold_labels() %>% add_stat_label()
```
...
## Table 3 A-B. Cytopenias following cilta-cel
Evaluable patients (who survived and did not progressed) are included for each time points.
```{r Table 3}
tbl1 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  ...
  tbl_summary(by = gr3_cytopenia_30d,
              type = list(
                c(extramedullary_disease_yes_no,
                  ...
                  did_patient_meet_eligibility_criteria_for_cartitude_1_on_the_day_of_apheresis_yes_no)
                ~"categorical",
                c(number_of_prior_lines_of_therapy, `Marrow Cellularity (%) at Day30`)
                ~ "continuous"),
              statistic = list(all_continuous() ~ "{median} ({min}, {max})"), 
              digits = all_continuous() ~ 1
  ) %>% 
...
## Table S4b. Patient characteristics by ANY early ICAHT
```{r Patient characteristics by grade 3+ early ICAHT}
cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  # filter(is.na(Not_evaluable_at_30days)) %>% ######################### Fix evaluable----------
  select(circulating_plasma_cell,
         ...
         ht_score,
         any_early_icaht) %>% 
  tbl_summary(by = any_early_icaht,
              type = list(
                c(extramedullary_disease_yes_no,
                  cytopenia_apheresis,`cytopenia_-5d`,
                  cytogenetics, cytogenetics_incl_1q,
  ...
  bold_labels() %>% add_stat_label() %>% add_p() %>% 
  modify_spanning_header(all_stat_cols() ~ "**Grade 3+ Early ICAHT**")
```

## Table S5a. Patient characteristics by grade 3+ cytopenia at Day 90
...
cilta %>% 
  ...
  select(circulating_plasma_cell,
         ...
         extramedullary_disease_yes_no, 
         high_marrow_burden_50_percent, ecog_at_ld, r_iss_at_car_t_infusion, 
         cytogenetics, cytogenetics_incl_1q,
         bridging_therapy_yes_no,
         # `Received alkylating therapy with bridging (Yes=1, No=0)`,
         prior_auto_sct_yes_no, prior_allo_sct_within_6_months_before_apheresis,
  ...
  tbl_summary(by = gr3_cytopenia_90d,
              type = list(
                c(extramedullary_disease_yes_no,
                  ...
                  did_patient_meet_eligibility_criteria_for_cartitude_1_on_the_day_of_apheresis_yes_no)
                ~ "categorical",
                c(number_of_prior_lines_of_therapy, `Marrow Cellularity (%) at Day90`)
                ~ "continuous"),
              statistic = list(all_continuous() ~ "{median} ({min}, {max})"), 
              digits = all_continuous() ~ 1
  ) %>% 
...
## Table S5b. Patient characteristics by ANY late ICAHT
```{r Patient characteristics by grade 3+ late ICAHT}
cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  # filter(is.na(Not_evaluable_at_90days)) %>%  ######################### Fix evaluable----------
  select(circulating_plasma_cell,
  ...
  bold_labels() %>% add_stat_label() %>%  add_p() %>% 
  modify_spanning_header(all_stat_cols() ~ "**Grade 3+ Late ICAHT**")
```


## Table S6a. CRS, ICANS, tocilizumab use, steroid use, anakinra use and bridging therapy by grade 3+ cytopenia at Day 30
...
## Table S6b. CRS, ICANS, tocilizumab use, steroid use, anakinra use and bridging therapy by ANY early ICAHT
```{r CRS, ICANS, tocilizumab use, steroid use, anakinra use and bridging therapy by grade 3+ early ICAHT}
tbl1 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  # filter(is.na(Not_evaluable_at_30days)) %>% ######################### Fix evaluable----------
  select(cell_dose_million_cells,
...
tbl2 <- cilta %>% 
  ...
  # filter(is.na(Not_evaluable_at_30days)) %>% ######################### Fix evaluable----------
  select(steroid_use_yes_no, tocilizumab_yes_no, 
         anakinra_yes_no, any_early_icaht) %>% 
  tbl_summary(by = any_early_icaht,
              type = list(
    c(steroid_use_yes_no, tocilizumab_yes_no, 
      anakinra_yes_no) ~ "categorical"),
...
cilta %>% 
  ...
  tbl_summary() %>% 
  bold_labels() %>% add_stat_label() %>%
  modify_spanning_header(all_stat_cols() ~ "**Severe infections in the first 30 days**")
...
cilta %>% 
  ...
  tbl_summary() %>% 
  bold_labels() %>% add_stat_label() %>%
  modify_spanning_header(all_stat_cols() ~ "**Severe infections day 31 – 100**")
...
cilta %>% 
  ...
  tbl_summary() %>% 
  bold_labels() %>% add_stat_label() %>%
  modify_spanning_header(all_stat_cols() ~ "**Severe infections after day 100**")

```


## Table S8. Details of infections after cilta-cel
Patients can be counted multiple times if multiple infections.
...
## Table S9. Patients with infections by CRS, ICANS, tocilizumab use, steroid use, anakinra use and bridging therapy
...
## Table S10 A-B. Patients with infections by cell counts and immunoglobulin levels
...
## Table S10 C-D. Immunoglobulin levels excluding patients with that type of myeloma 
...
This will help for creating the figure. It shows the patients who had PD or died with info of the event.
```{r}
fig_dat %>% 
  mutate(pfs_from_infusion_days = 
           interval(
             start = date_of_car_t_infusion, 
             end = pfs_date)/
           duration(n=1, units = "days")) %>% 
  filter(os_event == 1 | pfs_event == 1) %>% 
  mutate(event_at = case_when(
    !is.na(date_of_pd) & pfs_from_infusion_months < 1        ~ "had pd",
    !is.na(date_of_pd) & pfs_from_infusion_months < 3        ~ "had pd",
    !is.na(date_of_pd) & pfs_from_infusion_months < 6        ~ "had pd",
    !is.na(date_of_pd) & pfs_from_infusion_months < 12       ~ "had pd",
    os_event == 1 & pfs_from_infusion_months < 1        ~ "died",
    os_event == 1 & pfs_from_infusion_months < 3        ~ "died",
    os_event == 1 & pfs_from_infusion_months < 6        ~ "died",
    os_event == 1 & pfs_from_infusion_months < 12       ~ "died",
  )) %>% 
  mutate(phrase = paste("Patient", pt_identifier, event_at, "at day", pfs_from_infusion_days)
  ) %>% 
  arrange(pfs_from_infusion_days) %>% 
  select(phrase, `cause of death` = cause_of_death_myeloma_toxicitiy_unrelated,
         `CRS grade` = max_crs_grade_0_5, max_icans, delayed_neuro, `delayed neurotoxicity` = delayed_neuro_type, parkinsonian_neurotoxicity_yes_no, what_non_parkinsonian_neurotoxicity, infection_type_bacterial_viral_fungal_parasitic,  `infection details` = infection_type_details)
...
a <- fig_dat %>% 
  ...
  filter(event_at == "died") %>% 
  arrange(pfs_from_infusion_days) %>% 
  select(pt_identifier, event_at, pfs_from_infusion_days, `cause of death` = cause_of_death_myeloma_toxicitiy_unrelated,
         `CRS grade` = max_crs_grade_0_5, max_icans, delayed_neuro, `delayed neurotoxicity` = delayed_neuro_type, parkinsonian_neurotoxicity_yes_no, what_non_parkinsonian_neurotoxicity, infection_type_bacterial_viral_fungal_parasitic,  `infection details` = infection_type_details)
write_csv(a, paste0(here::here(), "/cilta_cel/cause of death cilta-cel data.csv"))
```
...
c <- a %>% 
  ...
  mutate(day = factor(day, levels = c("Apheresis", "Day -5", "Day 30", "Day 60", "Day 90", "Day 180"))) %>% 
...
c %>% filter(grade == "Grade 3+") %>% 
  ...
  geom_bar(stat = "identity",
           position=position_dodge(width=0.9))+
  geom_text(aes(label = paste0(round(perc, 0), "%"), y=perc-2), size = 4,
            position = position_dodge(width=0.9), vjust=-1.5
            )+
  
  scale_fill_brewer(palette = "Spectral")+
  ylim(0, 60)+
  labs(fill = NULL, x= NULL, y = "Percent")+
  theme_classic(base_size = 9)+
  theme(legend.position="bottom", text = element_text(size = 16))

ggsave("Figure 3 Cytopenias over time_600dpi_1column_compress.tiff",
       dpi = 600, units = "in",
       width = 8,
...
c %>% filter(grade == "Grade 3+") %>% 
  ...
  geom_bar(stat = "identity",
           position=position_dodge(width=0.9))+
  geom_text(aes(label = paste0(round(perc, 0), "%"), y=perc-2), size = 7,
            position = position_dodge(width=0.9), vjust=-1.5
            )+
...
ggsave("Figure 3 Cytopenias over time_600dpi_2column_compress.tiff",
       dpi = 600, units = "in",
       width = 16,
...
## Figure 4. Cumulative incidence of infection and infection density
```{r Figure 5}
library(tidycmprsk)
library(ggsurvfit)
...
a <- cilta %>%
  mutate(os_from_infusion_days = os_from_infusion_months * 30.417) %>% 
  select(pt_identifier, any_infection, any_sevinfection, date_of_infection, 
         time_from_infusion_to_infection_days, infection_type_bacterial_viral_fungal_parasitic,
         infection_type_cat) %>% 
  mutate(any_sevinfection = case_when(
    any_sevinfection == "Yes"           ~ "Severe"
  )) %>% 
  mutate(any_infection = case_when(
    any_sevinfection == "Severe"        ~ NA_character_,
    any_infection == "Yes"              ~ "Not severe"
  )) %>% 
  pivot_longer(cols = c(any_infection, any_sevinfection), 
               names_to = "Infection", 
               values_to = "Infection severity") %>% 
  drop_na(`Infection severity`) %>% 
  mutate(infection_type_cat = str_replace(infection_type_cat, " \\+ ", " and ")) %>% 
  separate_wider_delim(cols = infection_type_cat, delim = " and ",
                       names = c("infection_type_cat1", "infection_type_cat2"), 
                       too_few = "align_start", too_many = "merge", 
                       cols_remove = TRUE) %>% 
  pivot_longer(cols = c(infection_type_cat1, infection_type_cat2), 
               names_to = NULL, 
               values_to = "Infection categories") %>% 
  drop_na(`Infection categories`) %>% 
  ggplot(aes(x = infection_type_bacterial_viral_fungal_parasitic, 
             y = time_from_infusion_to_infection_days))+
  geom_violin()+
  geom_jitter(aes(color = `Infection severity`), width = 0.2)+
  # geom_point(aes(color = `Infection severity`), position = "jitter")
  labs(x = NULL, y = "Infection onset (days)")+
  scale_color_manual(values = c("black", "red"))+
  scale_y_continuous(expand = c(0, 5), limits = c(0, 400))+
...
b <- cilta %>%
  mutate(os_from_infusion_days = os_from_infusion_months * 30.417) %>% 
  select(pt_identifier, any_infection, any_sevinfection, date_of_infection, 
         time_from_infusion_to_infection_days,
         infection_type_cat) %>% 
  mutate(any_sevinfection = case_when(
    any_sevinfection == "Yes"           ~ "Severe"
  )) %>% 
  ...
  pivot_longer(cols = c(any_infection, any_sevinfection), 
               ...
               values_to = "Infection severity") %>% 
  drop_na(`Infection severity`) %>% 
  mutate(infection_type_cat = str_replace(infection_type_cat, "and Respiratory infection", "and Respiratory infections")) %>% 
  mutate(infection_type_cat = str_replace(infection_type_cat, " \\+ ", " and ")) %>% 
  separate_wider_delim(cols = infection_type_cat, delim = " and ",
                       names = c("infection_type_cat1", "infection_type_cat2"), 
                       too_few = "align_start", too_many = "merge", 
  ...
  pivot_longer(cols = c(infection_type_cat1, infection_type_cat2), 
               ...
               values_to = "Infection categories") %>% 
  drop_na(`Infection categories`) %>% 
  ggplot(aes(x = `Infection categories`, 
             y = time_from_infusion_to_infection_days))+
  geom_violin()+
  geom_jitter(aes(color = `Infection severity`), width = 0.2)+
  # geom_point(aes(color = `Infection severity`), position = "jitter")
...
c <- cilta %>%
  ...
  pivot_longer(cols = c(any_infection, any_sevinfection),
               ...
               values_to = "Infection severity") %>%
  drop_na(`Infection severity`) %>%
  mutate(viral_category = str_replace(viral_category, ", ", " And ")) %>% 
  mutate(viral_category = str_replace(viral_category, "Rhinovius", "Rhinovirus")) %>% 
  mutate(viral_category = str_replace(viral_category, "parainfluenza", "Parainfluenza")) %>% 
  mutate(viral_category = str_replace(viral_category, "Metapneurmovirus", "Metapneumovirus")) %>% 
  mutate(viral_category = str_remove(viral_category, "\\? ")) %>% 
  separate_wider_delim(cols = viral_category, delim = " And ",
                       names = c("viral_category1", "viral_category2"), 
                       too_few = "align_start", too_many = "merge", 
                       cols_remove = FALSE) %>% 
  pivot_longer(cols = c(viral_category1, viral_category2), 
               names_to = NULL, 
               values_to = "Viral categories") %>% 
  drop_na(`Viral categories`) %>% 
  ggplot(aes(x = `Viral categories`,
             y = time_from_infusion_to_infection_days))+
  geom_violin()+
  geom_jitter(aes(color = `Infection severity`), width = 0.2)+
  labs(x = NULL, y = "Infection onset (days)")+
...
ggsave("Figure 4 Infection density_600dpi_1column_compress.tiff",
       dpi = 600, units = "in",
       width = 8,
       height = 11, compression = "lzw")
...
alluvial_A <- moffitt_subset %>% 
  ...
  scale_fill_discrete(name = "HSV 1/2 IgG")+
...
alluvial_B <- moffitt_subset %>% 
  ...
  summarise(count=n()) %>% 
  mutate(vzv_ig_g_apheresis = factor(vzv_ig_g_apheresis, levels = c("Positive", "Negative"))) %>% 
  
  ggplot(aes(axis1 = vzv_ig_g_apheresis, axis2= vzv_ig_g_d90,
             y = count)) +
  geom_alluvium(aes(fill = vzv_ig_g_apheresis), 
                color = c("transparent", "white", "transparent", "transparent", "transparent", "transparent")
  )+
  geom_stratum()+
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_fill_discrete(name = "VZV IgG")+
...
alluvial_C <- moffitt_subset %>% 
  ...
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_fill_discrete(name = "CMV IgG")+
  ggtitle("Evolution of virus titers")
```
...
ggsave("Figure S3. Viral titers after cilta-cel_600dpi_1column.tiff",
       dpi = 600, units = "in",
       width = 8,
...
ggsave("Figure S3+day180 Viral titers after cilta-cel_600dpi_1column_compress.tiff",
       dpi = 600, units = "in",
       width = 8,
       height = 10, compression = "lzw")
```

## Figure S4. Pneumococcal titers after cilta-cel
...
```{r Figure S2. Pneumococcal titers after cilta-cel, fig.width=15, fig.height=12}
library(ComplexHeatmap)
mat <- moffitt_subset %>% 
  select(pt_identifier, 
         intersect(starts_with("pneumo"), ends_with("change"))) %>% 
  rename(PolyAb3 = pneumo_poly_ab3_change, PolyAb6 = pneumo_poly_ab6_change, 
         PolyAb14 = pneumo_poly_ab14_change, PolyAb19 = pneumo_poly_ab19_change,
         PolyAb23 = pneumo_poly_ab23_change, Serotype1 = pneumo_serotype1_ig_g_change,
         Serotype4 = pneumo_serotype_4_ig_g_change, Serotype5 = pneumo_serotype5_ig_g_change,
         Serotype7F = pneumo_serotype7f_ig_g_change, Serotype8 = pneumo_serotype8_ig_g_change,
         Serotype9N = pneumo_serotype9n_ig_g_change, Serotype9V = pneumo_serotype9v_ig_g_change,
         Serotype12F = pneumo_serotype12f_ig_g_change, Serotype18C = pneumo_serotype18c_ig_g_change
         ) %>% 
  drop_na() %>% 
  mutate(new_sort =
...
ht_list <- Heatmap(df, name = " ", 
        rect_gp = gpar(col = "grey", lwd = 0.001), 
        ...
        col = colors)
ht_list
# pdf("Figure S4 Pneumococcal titers after cilta-cel.pdf", 
#     width = 8,
#     height = 6)
# draw(ht_list, heatmap_legend_side = "right")
# dev.off()

tiff("Figure S4 Pneumococcal titers after cilta-cel_600dpi_1column.tiff", 
     res = 600, units = "in",
    width = 8,
    height = 5)
ht_list
...
ragg::agg_tiff("ht_list.tiff", width = 8,
    height = 5, units = "in", res = 600, compression = "lzw")
ht_list
dev.off()
...
## Table S7 and Figure S2. MVA and forest plot
Limit to evaluable patients at day 90 (For Grade 3+ Cytopenia at days 90 MVA)
...
glm(gr3_cytopenia_90d ~ high_marrow_burden_50_percent + ht_score + 
      number_of_prior_lines_of_therapy,
    data = mva_dat,
    family = "binomial") %>%
...
glm(any_late_icaht ~ high_marrow_burden_50_percent + ht_score + 
      number_of_prior_lines_of_therapy,
    data = mva_dat_icaht,
    family = "binomial") %>%
  tbl_regression(exponentiate = TRUE, 
                 intercept = TRUE) %>%
  add_nevent(location = "level") %>% add_n(location = "level") %>%
  modify_spanning_header(everything() ~ "**Any late ICAHT**") %>%
  bold_p(t = .05)
glm(any_late_icaht ~ high_marrow_burden_50_percent + ht_score,
    data = mva_dat_icaht,
    family = "binomial") %>%
...
ggsave("Figure S2 Forest plot 5 variables_600dpi_1column_wraplabel_compress.tiff",
       dpi = 600, units = "in",
       width = 8,
       height = 4, compression = "lzw"#6
       )
...
# ggsave("Figure S2 Forest plot 3 variables.pdf", 
#        device = cairo_pdf,
#        width = 7,
#        height = 5)
```
...
library(survival)
myplot <- ggsurvplot(survfit(Surv(pfs_from_infusion_months, pfs_event) ~ gr3_cytopenia_90d, 
                   data=surv_dat),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
           ...
           legend = "top",
           legend.title = "≥ Grade 3 cytopenia at D90",
...
risk.table.ylab <- "≥ Grade 3 \ncytopenia at D90"
myplot$table <- myplot$table + labs(y = risk.table.ylab)
myplot

# ggsave("Survival plot by gr3 cytopenia day 90.pdf", 
#        width = 7,
#        height = 5)

fig_surv_cytday90 <- ggsave_workaround(myplot)
# 
# ggsave("Survival plot by gr3 cytopenia day 90 - risk table.pdf",
#        plot = fig_surv_cytday90,
#        path= paste0(here::here(), "/cilta_cel"),
#        width = 6, height = 6,
#        units = c("in"),
#        dpi = 600)
...
myplot <- ggsurvplot(survfit(Surv(pfs_from_infusion_months, pfs_event) ~ ORR_as_best_response, 
                   data=surv_dat),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
...
myplot
# ggsave("Survival plot by ORR as best response.pdf", 
#        width = 7,
#        height = 5)

fig_surv_ORR <- ggsave_workaround(myplot)
# 
# ggsave("Survival plot by ORR as best response - risk table.pdf",
#        plot = fig_surv_ORR,
#        path= paste0(here::here(), "/cilta_cel"),
#        width = 6, height = 6,
#        units = c("in"),
...
## Survival analysis - KM by gr3 cytopenia at day 30
```{r Survival analysis - KM by gr3_cytopenia_30d}
myplot <- ggsurvplot(survfit(Surv(pfs_from_infusion_months, pfs_event) ~ gr3_cytopenia_30d, 
                   data=surv_dat),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
           ...
           ylab = "PFS (probability)",
           legend = "top",
           legend.title = "≥ Grade 3 cytopenia at D30",
           legend.labs = c("No", "Yes"),
           pval = TRUE,
           conf.int = FALSE,
           # Censor
...
myplot$table <- myplot$table + labs(y = risk.table.ylab)
myplot
# ggsave("Survival plot by gr3 cytopenia day 30.pdf", 
#        width = 7,
#        height = 5)

fig_surv_cytday30 <- ggsave_workaround(myplot)
# 
# ggsave("Survival plot by gr3 cytopenia day 30 - risk table.pdf",
#        plot = fig_surv_cytday30,
#        path= paste0(here::here(), "/cilta_cel"),
#        width = 6, height = 6,
#        units = c("in"),
#        dpi = 600)
...
coxph(Surv(time = surv_dat$pfs_from_infusion_months, 
                   event = surv_dat$pfs_event) ~ gr3_cytopenia_30d, 
              data = surv_dat)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")

# ggsurvplot(survfit(Surv(os_from_infusion_months, os_event) ~ gr3_cytopenia_30d, 
#                    data=surv_dat),
#            title = "OS Analysis",
#            font.main = c(20, "bold", "black"),
#            font.x = c(15, "bold", "black"),
...
# 
# coxph(Surv(time = surv_dat$os_from_infusion_months, 
#                    event = surv_dat$os_event) ~ gr3_cytopenia_30d, 
#               data = surv_dat)  %>% 
#   tbl_regression(exponentiate = TRUE) %>% 
#   bold_p(t = .05) %>% 
#   add_nevent(location = "level") %>% add_n(location = "level")
...
## Survival analysis - KM by CAR-HEMATOTOX score
```{r Survival analysis - KM by ht_score}
myplot <- ggsurvplot(survfit(Surv(pfs_from_infusion_months, pfs_event) ~ ht_score, 
                   data=surv_dat),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
...
myplot
# ggsave("Survival plot by HTscore.pdf", 
#        width = 7,
#        height = 5)

fig_surv_HTscore <- ggsave_workaround(myplot)
# 
# ggsave("Survival plot by HTscore - risk table.pdf",
#        plot = fig_surv_HTscore,
#        path= paste0(here::here(), "/cilta_cel"),
#        width = 6, height = 6,
#        units = c("in"),
#        dpi = 600)
...
coxph(Surv(time = surv_dat$pfs_from_infusion_months, 
                   event = surv_dat$pfs_event) ~ ht_score, 
              data = surv_dat)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")

# ggsurvplot(survfit(Surv(os_from_infusion_months, os_event) ~ ht_score, 
#                    data=surv_dat),
#            title = "OS Analysis",
#            font.main = c(20, "bold", "black"),
#            font.x = c(15, "bold", "black"),
...
# 
# coxph(Surv(time = surv_dat$os_from_infusion_months, 
#                    event = surv_dat$os_event) ~ ht_score, 
#               data = surv_dat)  %>% 
#   tbl_regression(exponentiate = TRUE) %>% 
#   bold_p(t = .05) %>% 
#   add_nevent(location = "level") %>% add_n(location = "level")
...
## Survival analysis - KM by ANY early ICAHT
```{r Survival analysis - KM by gr3_early_icaht}
myplot <- ggsurvplot(survfit(Surv(pfs_from_infusion_months, pfs_event) ~ any_early_icaht,
                   data=surv_dat),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
...
myplot
# ggsave("Survival plot by any early ICAHT.pdf", 
#        width = 7,
#        height = 5)

fig_surv_earlyicaht <- ggsave_workaround(myplot)
# 
# ggsave("Survival plot by any early ICAHT - risk table.pdf",
#        plot = fig_surv_earlyicaht,
#        path= paste0(here::here(), "/cilta_cel"),
#        width = 6, height = 6,
#        units = c("in"),
#        dpi = 600)
...
coxph(Surv(time = surv_dat$pfs_from_infusion_months,
                   event = surv_dat$pfs_event) ~ any_early_icaht,
              data = surv_dat)  %>%
  tbl_regression(exponentiate = TRUE) %>%
  bold_p(t = .05) %>%
  add_nevent(location = "level") %>% add_n(location = "level")

# ggsurvplot(survfit(Surv(os_from_infusion_months, os_event) ~ gr3_early_icaht,
#                    data=surv_dat),
#            title = "OS Analysis",
#            font.main = c(20, "bold", "black"),
#            font.x = c(15, "bold", "black"),
...
#
# coxph(Surv(time = surv_dat$os_from_infusion_months,
#                    event = surv_dat$os_event) ~ gr3_early_icaht,
#               data = surv_dat)  %>%
#   tbl_regression(exponentiate = TRUE) %>%
#   bold_p(t = .05) %>%
#   add_nevent(location = "level") %>% add_n(location = "level")
...
## Survival analysis - KM by ANY late ICAHT
```{r Survival analysis - KM by gr3_late_icaht}
myplot <- ggsurvplot(survfit(Surv(pfs_from_infusion_months, pfs_event) ~ any_late_icaht,
                   data=surv_dat),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
...
myplot
# ggsave("Survival plot by any late ICAHT.pdf", 
#        width = 7,
#        height = 5)

fig_surv_lateicaht <- ggsave_workaround(myplot)
# 
# ggsave("Survival plot by any late ICAHT - risk table.pdf",
#        plot = fig_surv_lateicaht,
#        path= paste0(here::here(), "/cilta_cel"),
#        width = 6, height = 6,
#        units = c("in"),
#        dpi = 600)
...
coxph(Surv(time = surv_dat$pfs_from_infusion_months,
                   event = surv_dat$pfs_event) ~ any_late_icaht,
              data = surv_dat)  %>%
  tbl_regression(exponentiate = TRUE) %>%
  bold_p(t = .05) %>%
  add_nevent(location = "level") %>% add_n(location = "level")

# ggsurvplot(survfit(Surv(os_from_infusion_months, os_event) ~ gr3_late_icaht,
#                    data=surv_dat),
#            title = "OS Analysis",
#            font.main = c(20, "bold", "black"),
#            font.x = c(15, "bold", "black"),
...
#
# coxph(Surv(time = surv_dat$os_from_infusion_months,
#                    event = surv_dat$os_event) ~ gr3_late_icaht,
#               data = surv_dat)  %>%
#   tbl_regression(exponentiate = TRUE) %>%
#   bold_p(t = .05) %>%
#   add_nevent(location = "level") %>% add_n(location = "level")
...
```{r}
ggarrange(fig_surv_HTscore, fig_surv_cytday30,
          fig_surv_earlyicaht, fig_surv_cytday90,
          fig_surv_lateicaht,
          ncol = 3, nrow = 2,
          labels="AUTO")

ggsave("Survival panel_600dpi_2column_compress.tiff", #device = cairo_pdf,
       # path = paste0(here::here(), "/cilta_cel"),
       width = 16, height = 11,
       # units = c("in"),
...
ggarrange(fig_surv_HTscore, fig_surv_cytday30,
          fig_surv_cytday90, fig_surv_earlyicaht, 
          fig_surv_lateicaht,
          ncol = 3, nrow = 2,
          labels="AUTO")

ggsave("Survival panel_600dpi_2column_reordered_compress.tiff", #device = cairo_pdf,
       # path = paste0(here::here(), "/cilta_cel"),
       width = 16, height = 11,
       # units = c("in"),
       dpi=600, compression = "lzw",
