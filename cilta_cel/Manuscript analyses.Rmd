...
vec_col <- c("bm_plasma_cell_percent_at_pre_ld",
             ...
             "ig_g_apheresis",
             "ig_g_day_5", "ig_g_day_30", "ig_g_day_90",
             "ig_a_day_5", "ig_a_day_30", "ig_a_day_90",
             "ig_m_day_5", "ig_m_day_30", "ig_m_day_90",
             
             "cd8_apheresis", "cd8_day_30", 
             "cd8_day_90", "cd8_day_180", 
             "cd19_apheresis", "cd19_day_30",
             "cd19_day_90", "cd19_day_180",
...
# has < : "CRP Pre-Leukapheresis", "IgG day -5", "IgA day -5", "IgM day -5"
# has < :"CRP day +21", "IgG day +30", "IgA day +30", "IgM day +30", "marrow_percent_cd138_day_30", "marrow % cellularity day +30", "CRP day +30", "CD19 day +90", "IgG day +90", "IgA day +90", "IgM day +90", "marrow %CD138 day +90", 
# # has > : "CD8 day +30", "CRP day +90"
# has < : "CD19 day +180", "IgG day +180", "IgA day +180", "IgM day +180", "marrow % cellularity day +180", "CRP day +180"
...
var_label(cilta) <- list(age = "Patient age", agecat = "Patient age - Categories", 
                         ...
                         need_for_g_csf_post_infus_yes_1_no_0 = "Need for gCSF post infusion",
                         need_for_ivig_yes_1_no_0 = "Need for IVIG",
...
tbl7 <- cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
  select(best_response, best_response_MRD) %>% 
  tbl_summary(#type = list(c(MRD_mon3) ~ "categorical"),
...
Jenn did it for the ide-cel paper

```{r}
# cart1 %>% 
#   distinct(pt_identifier, .keep_all = TRUE) %>% 
#   select(`need_for_g_csf_post_infus_yes_1_no_0`, `First Day of GCSF`, `Last Day of GCSF`,
#          `cd34_stem_cell_boost_yes_1_no_0`, `Day of stem cell boost`, `cell_dose_million_cells`,
#          `Engraftment Day`,
#          `rbc_transfusion_within_30d_post_cart_yes_1_no_0`, `plt_transfusion_within_30d_post_cart_yes_1_no_0`, 
#          `rbc_transfusion_30d_post_cart_yes_1_no_0`, `plt_transfusion_30d_post_cart_yes_1_no_0`,
#          `need_for_ivig_yes_1_no_0`, `First day of IVIG`, `Last day of IVIG`
#   ) %>% 
#   tbl_summary(type = list(
#     c(`Day of stem cell boost`, `cell_dose_million_cells`,
#     `Last day of IVIG`) ~ "continuous",
#     c(`need_for_g_csf_post_infus_yes_1_no_0`,
#       `cd34_stem_cell_boost_yes_1_no_0`,
#       `rbc_transfusion_within_30d_post_cart_yes_1_no_0`, `plt_transfusion_within_30d_post_cart_yes_1_no_0`, 
#          `rbc_transfusion_30d_post_cart_yes_1_no_0`, `plt_transfusion_30d_post_cart_yes_1_no_0`,
#       `need_for_ivig_yes_1_no_0`) ~ "categorical"
#   ),
#             statistic=list(all_continuous() ~ "{median} ({min}, {max})"), 
#             digits = all_continuous() ~ 1) %>% 
#   bold_labels() %>% add_stat_label()
...
tbl3 <- cilta %>% 
  ...
  select(#day_30_mrd_by_clono_seq_positive_vs_negative,
         ...
         day30_response_v2, gr3_cytopenia_30d) %>% 
  tbl_summary(by = gr3_cytopenia_30d,
    #           type = list(
...
tbl5 <- cilta %>% 
  ...
  select(#MRD_mon3,
         ...
         mon3_response_v2, gr3_cytopenia_30d) %>% 
  tbl_summary(by = gr3_cytopenia_30d,
    #           type = list(
    # c(MRD_mon3) ~ "categorical"),
    digits = all_continuous() ~ 1,
...
CCT5019-0512 has a time to infection but no in Any Infection but yes in Any Severe Infection
```{r}
cilta %>%
...
Should I limit to infections in the first 100 days?
```{r}
cilta %>% 
  distinct(pt_identifier, .keep_all = TRUE) %>% 
...
risk_comp <- cilta %>% 
  mutate(event_type = case_when(
    ...
    is.na(infection_type_bacterial_viral_fungal_parasitic)              ~ "None"
  )) %>% 
  mutate(event_type =
           factor(event_type, levels= c("None","Bacterial","Fungal", "Viral"))) %>%
...
cum_inc <- cuminc(Surv(date_of_event, event_type) ~ 1, data = risk_comp) %>% 
...
cuminc(Surv(date_of_event, event_type) ~ 1, data = risk_comp) %>% 
  ggcuminc(outcome = c("Bacterial", "Fungal", "Viral"), 
           geom_step(linewidth=10)) +
  ylim(c(0, 0.25)) +
  labs(
