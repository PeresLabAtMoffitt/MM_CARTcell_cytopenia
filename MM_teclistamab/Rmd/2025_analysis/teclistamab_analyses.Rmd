...
<!-- - There was text into the variable date_of_first_bs_ab_full_dose. The text was transferred into -->
<!-- reason_no_date_of_first_bs_ab_full_dose. -->
<!--   Patient with text have progressed. -->
<!-- - in_out_patient was coded "In" for patient with hospital length stay values and "Out" when no values as there is no 0 values. -->
<!-- - is_percent_cp_cs_checked_within_30_days_before_bs_ab_initiation is to record when %CPC was checked. So it was -->
<!-- not checked when percent_cp_cs_if_pb_checked_within_30_days_before_bs_ab_initiation was 0. -->
<!-- The 0 were then removed from percent_cp_cs_if_pb_checked_within_30_days_before_bs_ab_initiation. -->
<!-- Is it correct? -->
<!-- - The patient who had a PD date at 2024-02-27 and discontinuation at the same date was marked as PD for the day 30 response. -->
<!-- - Cytopenia is coded Yes for baseline ANC < 1.8 or baseline Hgb < 11.4 or baseline Plt < 143 and No if all the baseline values are >. -->
<!-- - Grade 3 cytopenia is coded Yes for baseline ANC < 1 or baseline Hgb < 8 or baseline Plt < 50 and No if all the baseline values are >. -->
<!-- - infection_severity has a "N/a" and "mild" values, they were coded as not severe.     -->
<!-- - anc_nadir_value_during_first_6_cycles_k_u_l_ie_100_200_etc has a vale of 22.28 which seems pretty high...     -->
<!-- - 1 patient with a viral infection is coded No for infection_n_0_y_1. Was recoded Yes. -->

<!-- Issues in data with no impact for now: -->

<!-- - date_of_max_ferritin has a date "20-Sep". -->
<!-- - Difference between prior_bcma_directed_therapy_no_0_y_1 and prior_bcma_directed_therapy_yes_no. BEFORE : Looks like the yes/no is not recoded well so I am using 0/1 that I recoded myself. NOW THERE IS A DIFFERENCE IN 1 PATIENT. 27/52 vs 28/50....      -->
...
<!-- - prior_auto_sct_no_0_yes_1 has a "9" value and was recoded as NA. -->
<!-- - Most recent BCMA agent name (Carvylti should be Carvykti) -->
...
<!-- should I combine "CRS and Infection" and "Infection and CRS"? others? or is there a time order to consider? -->
...
var_label(bsab_data) <- list(age_at_bs_ab_initiation = "Patient age", agecat = "Patient age - Categories", 
                         ...
                         # `ECOG > or = 2? (Yes, No)` ,
                         time_from_MM_to_tec = "Time from MM diagnosis to BsAb",
...
bsab_data %>% 
  ...
  tbl_summary(by = ivig_n_0_y_1,
              type = list(days_of_icu_admission ~ "continuous",
...
long_infection_data <- bsab_data %>%
  ...
  pivot_longer(cols = -c(pt_identifier, #date_of_bs_ab_step_up_d1,
                         ivig_n_0_y_1, time_from_bsab_therapy_to_1st_infection_days,
                         infection_n_0_y_1#, number_of_infections
                         ),
...
long_infection_data %>%
  ...
  distinct() %>%
  group_by(pt_identifier, time_from_bsab_therapy_to_1st_infection_days,
           ivig_n_0_y_1,
...
Median OS
```{r}
survfit(Surv(os_from_initiation_months, os_event) ~ 1, data = bsab_data)
survfit(Surv(os_from_initiation_months, os_event) ~ ivig_n_0_y_1, data = bsab_data)
```

x-month survival probability
```{r}
summary(survfit(Surv(os_from_initiation_months, os_event) ~ 1, data = bsab_data), times = c(3,6,9,12))

summary(survfit(Surv(os_from_initiation_months, os_event) ~ ivig_n_0_y_1, data = bsab_data), times = c(3,6,9,12))
```


```{r Survival analysis - KM by ivig_n_0_y_1}
ggsurvplot(survfit(Surv(os_from_initiation_months, os_event) ~ ivig_n_0_y_1, 
                   data = bsab_data),
           # title = "PFS Analysis",
           ...
           ylab = "OS (probability)",
           legend = "top",
           legend.title = "IVIG", legend.labs = c("No", "Yes"),
           pval = TRUE,
           conf.int = FALSE,
           # Censor
           censor = TRUE,
...
coxph(Surv(time = bsab_data$os_from_initiation_months, 
                   event = bsab_data$os_event) ~ ivig_n_0_y_1, 
              data = bsab_data)  %>% 
...
Median PFS
```{r}
survfit(Surv(pfs_from_initiation_months, pfs_event) ~ 1, data = bsab_data)
survfit(Surv(pfs_from_initiation_months, pfs_event) ~ ivig_n_0_y_1, data = bsab_data)
```

x-month PFS probability
```{r}
summary(survfit(Surv(pfs_from_initiation_months, pfs_event) ~ 1, data = bsab_data), times = c(3,6,9,12))

summary(survfit(Surv(pfs_from_initiation_months, pfs_event) ~ ivig_n_0_y_1, data = bsab_data), times = c(3,6,9,12))
```

```{r PFS analysis - KM by ivig_n_0_y_1}
ggsurvplot(survfit(Surv(pfs_from_initiation_months, pfs_event) ~ ivig_n_0_y_1, 
                   data = bsab_data),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
...
coxph(Surv(time = bsab_data$pfs_from_initiation_months, 
                   event = bsab_data$pfs_event) ~ ivig_n_0_y_1, 
              data = bsab_data)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level") %>% as_gt()
...
```{r}
coxph(Surv(time = bsab_data$os_from_initiation_months, 
                   event = bsab_data$os_event) ~ ivig_n_0_y_1 + cytogenetics + high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1, 
              data = bsab_data)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
...
# glm(os_event ~ ivig_n_0_y_1 + cytogenetics + high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1,
#     data = bsab_data,
#     family = "binomial") %>%
#   tbl_regression(exponentiate = TRUE, 
#                  intercept = TRUE) %>%
...
coxph(Surv(time = bsab_data$pfs_from_initiation_months, 
                   event = bsab_data$pfs_event) ~ ivig_n_0_y_1 + cytogenetics + high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1, 
              data = bsab_data)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level") %>% 
...
# glm(pfs_event ~ ivig_n_0_y_1 + cytogenetics + high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1,
#     data = bsab_data,
#     family = "binomial") %>%
#   tbl_regression(exponentiate = TRUE, 
#                  intercept = TRUE) %>%
