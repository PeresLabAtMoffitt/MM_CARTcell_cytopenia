...
data <- 
  read_csv(#locale = locale(encoding="latin1"),
    paste0(#path, 
           here(), 
           "/Tec_07152025.csv"),
    na = c("N/A", " ", "") # N/a in infection severity + to fix the weird encoding with non-breaking space
    ) %>% 
...
data <- data %>% 
  ...
  `colnames<-`(str_remove(colnames(.), "~NA") %>% 
                 str_remove(., "_bacterial_viral_fungal")
               ) %>% 
  janitor::clean_names() %>% 
  `colnames<-`(str_remove(colnames(.), "_severe_hospitalization_or_requiring_iv_antibiotics")) %>% 
  slice(-1)
data_ <- data %>% 
  rename(pt_identifier = patient_identifier_use_same_id_for_cns_amyloid_sheets_if_appropriate_1,
         ...
         date_of_infection_1st = date_of_infection_76,
         infection_severity_1st = infection_severity_75,
         date_of_infection_2nd = date_of_infection_79,
         infection_severity_2nd = infection_severity_78,
         date_of_infection_3rd = date_of_infection_82,
         infection_severity_3rd = infection_severity_81,
         date_of_infection_4th = date_of_infection_85,
         infection_severity_4th = infection_severity_84,
         # ivig_n_0_y_1 = ivig_use_yes_1_no_1,
         ivig_cat = primary_prophy_0_secondary_prophy_1
         # hgb_nadir_date = hgb_nadir_value
         )

data1 <- type.convert(data_, as.is = TRUE)
```
...
data1 <- temp_fix %>% 
  # General----
  mutate(pt_identifier = as.character(pt_identifier)) %>% 
  mutate(end_of_data_extraction = as.Date("2025-06-30")) %>% 
  ...
  mutate(ivig_cat = case_when(
    ivig_n_0_y_1 == "No"       ~ "No IVIG",
    ivig_cat == 0              ~ "Primary IVIG",
    ivig_cat == 1              ~ "Secondary IVIG"
  )) %>% 
  ...
  mutate(raceeth = case_when(
    hispanic_1_if_hispanic_latin_x_or_spanish_origin_0_if_no_unknown == "Hispanic, latinX, spanish origin"
    ~ "Hispanic any races",
    race_white_0_black_1_aapi_2_american_indian_3_other_4_unk_5 == "White" &
      hispanic_1_if_hispanic_latin_x_or_spanish_origin_0_if_no_unknown == "No/Unknown"
    ~ "White Non-Hispanic",
    race_white_0_black_1_aapi_2_american_indian_3_other_4_unk_5 == "Black" &
      hispanic_1_if_hispanic_latin_x_or_spanish_origin_0_if_no_unknown == "No/Unknown"
    ~ "Black Non-Hispanic",
    !is.na(race_white_0_black_1_aapi_2_american_indian_3_other_4_unk_5) &
      hispanic_1_if_hispanic_latin_x_or_spanish_origin_0_if_no_unknown == "No/Unknown"
    ~ "Other Non-Hispanic",
    TRUE          ~ race_white_0_black_1_aapi_2_american_indian_3_other_4_unk_5
  )) %>% 
  ...
  # mutate(infection_1_type) %>%
  
  
  mutate(infection_severity_1st = case_when(
    infection_severity_1st == "N/a"        ~ NA_character_,
    TRUE       ~ infection_severity_1st
  )) %>%
  ...
  mutate(treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays = str_replace(
    treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays, 
    "1", "Infection")) %>% 
  mutate(treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays = str_replace(
    treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays, 
    "2", "Cytopenias")) %>% 
  ...
  mutate(treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays = str_replace(
    treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays, 
    "5", "Other")) %>% 
  # mutate(treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays = case_when(
    # treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays == 1        ~ "Infection",
    # treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays == 2        ~ "Cytopenias",
    # treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays == 3        ~ "CRS",
    # treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays == 4        ~ "ICANS",
    # treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays == 5        ~ "Other",
  #   TRUE                                      ~ treatment_delays_during_first_cycle_infection_1_cytopenias_2_crs_3_icans_4_other_5_explain_leave_blank_if_no_delays
  # )) %>% 
  ...
  mutate(just_prior_to_bs_ab_initiation_is_there_true_emd = case_when(
  ...
  mutate(true_emd_prior_bsab = case_when(
    just_prior_to_bs_ab_initiation_is_there_true_emd == "True EMD (+/-paraskeletal)"     ~ "Yes",
    just_prior_to_bs_ab_initiation_is_there_true_emd == "Paraskeletal only"              ~ "No",
    just_prior_to_bs_ab_initiation_is_there_true_emd == "None"                           ~ "No"
  )) %>% 
...
cart4 <- cart3 %>%   
  ...
  mutate(best_VGPRorbetter = case_when(
     VGPRorbetter_mon3 == "VGPR or better" | 
       VGPRorbetter_30days == "VGPR or better"
                                                                   ~ "VGPR or better",
     is.na(VGPRorbetter_mon3) & 
       is.na(VGPRorbetter_30days)
                                                                   ~ NA_character_, 
     TRUE                                                         ~ "< VGPR"
   ), .after = CRorbetter_mon3) %>%
...
bsab_data <- cart4 %>%
  ...
  mutate(time_from_bsab_therapy_to_1st_infection_days = 
         ...
         time_from_bsab_therapy_to_1st_infection_days = case_when(
    time_from_bsab_therapy_to_1st_infection_days >= 0     ~ time_from_bsab_therapy_to_1st_infection_days,
    TRUE                                          ~ NA_real_
  ...
  mutate(time_from_bsab_therapy_to_1st_infection_months = 
           ...
           duration(n=1, units = "months"), 
         # no need of the following for this data but keep in case
         time_from_bsab_therapy_to_1st_infection_months = case_when(
    time_from_bsab_therapy_to_1st_infection_months >= 0     ~ time_from_bsab_therapy_to_1st_infection_months,
    TRUE                                          ~ NA_real_
    ), .after = date_of_infection_1st) %>%
...
var_label(bsab_data) <- list(age_at_bs_ab_initiation = "Patient age", agecat = "Patient age - Categories", 
                         ...
                         # `ECOG > or = 2? (Yes, No)` ,
                         time_from_MM_to_tec_days = "Time from MM diagnosis to BsAb (days)",
                         ...
                         infection_n_0_y_1 = "Any Infection",
                         ...
                         infection_severity_1st = "Infection severity (1st)",
                         infection_type_2 = "Infection type (2nd)",
                         infection_severity_2nd = "Infection severity (2nd)",
                         infection_type_3 = "Infection type (3rd)",
                         infection_severity_3rd = "Infection severity (3rd)",
                         infection_type_4 = "Infection type (4th)",
                         infection_severity_4th = "Infection severity (4th)",
                         ...
                         day30_response = "Day 30 Response (Detailed)",
                         ...
                         mon3_response = "Day 90 Response (Detailed)",
...
bsab_data %>% 
  select(age_at_bs_ab_initiation, agecat, 
         ...
         year_bsab, 
         time_from_MM_to_tec_days,
         duration_of_tec_days, #duration_of_tec_days_cat,
         time_from_tec_to_end_extraction_days,
         ivig_n_0_y_1, ivig_cat,
         
         oligo_or_non_secretory_at_bs_ab_initiation_0_no_1_yes, myeloma_involved_heavy_chain_ig_g_ig_a_ig_m_ig_d_ig_e_or_blank, 
         myeloma_involved_light_chain_kappa_lambda_biclonal_blank, 
         `Subtype` = subtype,
         high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1,
         # cytogenetics, 
         ...
         immuno, #daratumumab_or_isatuximab_status_exposed_0_vs_refractory_1_vs_naive_2, 
         dara, 
         doubleref, 
         triple_class_refractory_i_mi_d_pi_anti_cd38_no_0_y_1, 
         penta_refractory_2_p_is_2_i_mi_ds_anti_cd38_no_0_y_1,
  ...
  tbl_summary(by = ivig_cat,
              type = list(c(days_of_icu_admission, 
                            if_inpatient_length_of_hospital_stay_total_days_including_all_admissions_0_if_done_outpt_with_no_admissions,
                            number_of_prior_lines_of_therapy_before_bs_ab
                            ) ~ "continuous",
                          ecog_ps_at_bs_ab_initiation_0_5 ~ "categorical",
  ...
  modify_spanning_header(all_stat_cols() ~ "Intravenous immunoglobulin (IVIG)") %>% 
...
```{r table 4}
bsab_data %>% 
  select(#is_percent_cp_cs_checked_within_30_days_before_bs_ab_initiation,
         percent_cp_cs_if_pb_checked_within_30_days_before_bs_ab_initiation,
         ...
         cytopenia_baseline, gr3_cytopenia_baseline,
         baseline_alc_k_u_l_ie_100_200_etc, alc_day_8, alc_day_15, alc_day_22, alc_c2d1, 
         ivig_cat) %>%
  tbl_summary(by = ivig_cat,
              type = list(
                c(percent_cp_cs_if_pb_checked_within_30_days_before_bs_ab_initiation) ~ "continuous",
                all_dichotomous() ~ "categorical"),
              statistic = list(all_continuous() ~ "{median} ({min}, {max})"), 
              digits = all_continuous() ~ 1
  ) %>% 
...
bsab_data %>% 
  select(max_crs_grade_0_5, CRS_any, #crs_any_coded_0_1, 
         ...
         day_of_max_crs_relative_to_initiation, 
         max_icans_grade, ICANS_any, #ICANS_any2, 
         ICANS_gradecat, gr2_ICANS, 
         # gr3_ICANS, 
...
long_infection_data <- bsab_data %>%
  select(pt_identifier, #date_of_bs_ab_step_up_d1,
         ...
         infection_type_1,
         infection_severity_1st,
         infection_type_2, infection_severity_2nd,
         infection_type_3, infection_severity_3rd,
         infection_type_4, infection_severity_4th,
         ivig_cat) %>%
  ...
  pivot_longer(cols = -c(pt_identifier, #date_of_bs_ab_step_up_d1,
                         ivig_cat, time_from_bsab_therapy_to_1st_infection_days,
                         time_from_bsab_therapy_to_1st_infection_months,
                         pfs_time_from_initiation_months,
                         infection_n_0_y_1#, number_of_infections
                         ),
...
```{r}
long_infection_data %>% filter(any_infection == "Yes") %>% 
  mutate(any_severe_infection = case_when(
    any_severe_infection == "Severe"              ~ "Yes",
    !is.na(any_infection)                         ~ "No"
  )) %>%
  select(`Infection Type` = infection_type, `Any severe infection` = any_severe_infection, `Infection details` = pathogen_)
```
...
```{r Table 8}
bsab_data %>% 
  select(progression_yes_no, pfs_time_from_initiation_months, #pfs_event, 
...
Infection date is unknown for two patients (1 primary IVIG, 1 secondary IVIG) and were excluded from the CI analyses.
```{r}
library(tidycmprsk)
library(ggsurvfit) # for ggcuminc

risk_comp <- long_infection_data %>%
  select(pt_identifier, 
         any_infection, #any_severe_infection,
         time_from_bsab_therapy_to_1st_infection_months,
         total_number_of_infection,
         pfs_time_from_initiation_months,
         # infection_type,
  ...
  mutate(event_type = factor(event_type, levels = c("0", "1"))) %>% 

  mutate(event_time = coalesce(time_from_bsab_therapy_to_1st_infection_months, pfs_time_from_initiation_months)) %>% 
  mutate(event_time = case_when(
    any_infection == "Yes" &
      is.na(time_from_bsab_therapy_to_1st_infection_months)            ~ NA_real_,
    !is.na(event_time)                                                 ~ event_time
  ))
# 
...
```{r}
cuminc(Surv(event_time, event_type) ~ 1, data = risk_comp)
...
cuminc(Surv(event_time, event_type) ~ ivig_cat, data = risk_comp)
...
# Survival by IVIG Primary/Secondary
## OS
Median OS
```{r}
survfit(Surv(os_time_from_initiation_months, os_event) ~ 1, data = bsab_data)
survfit(Surv(os_time_from_initiation_months, os_event) ~ ivig_cat, data = bsab_data)
```

x-month survival probability
```{r}
summary(survfit(Surv(os_time_from_initiation_months, os_event) ~ 1, data = bsab_data), times = c(6, 12, 18, 24))

summary(survfit(Surv(os_time_from_initiation_months, os_event) ~ ivig_cat, data = bsab_data), times = c(6, 12, 18, 24))
```


```{r Survival analysis - KM by ivig_cat}
ggsurvplot(survfit(Surv(os_time_from_initiation_months, os_event) ~ ivig_cat, 
                   data = bsab_data),
           # title = "PFS Analysis",
           ...
           ylab = "OS (probability)",
           legend = "top",
           # legend.title = "IVIG", legend.labs = c("No IVIG", "Primary", "Secondary"),
           pval = TRUE,
           conf.int = FALSE,
           # Censor
           censor = TRUE,
...
coxph(Surv(time = bsab_data$os_time_from_initiation_months, 
                   event = bsab_data$os_event) ~ ivig_cat, 
              data = bsab_data)  %>% 
...
Median PFS
```{r}
survfit(Surv(pfs_time_from_initiation_months, pfs_event) ~ 1, data = bsab_data)
survfit(Surv(pfs_time_from_initiation_months, pfs_event) ~ ivig_cat, data = bsab_data)
```

x-month PFS probability
```{r}
summary(survfit(Surv(pfs_time_from_initiation_months, pfs_event) ~ 1, data = bsab_data), times = c(6, 12, 18, 24))

summary(survfit(Surv(pfs_time_from_initiation_months, pfs_event) ~ ivig_cat, data = bsab_data), times = c(6, 12, 18, 24))
```

```{r PFS analysis - KM by ivig_cat}
ggsurvplot(survfit(Surv(pfs_time_from_initiation_months, pfs_event) ~ ivig_cat, 
                   data = bsab_data),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
...
coxph(Surv(time = bsab_data$pfs_time_from_initiation_months, 
                   event = bsab_data$pfs_event) ~ ivig_cat, 
              data = bsab_data)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level") %>% as_gt()
...
<!-- ```{r} -->
<!-- coxph(Surv(time = bsab_data$os_time_from_initiation_months,  -->
<!--                    event = bsab_data$os_event) ~ ivig_cat + cytogenetics + high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1,  -->
<!--               data = bsab_data)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
<!--   add_nevent(location = "level") %>% add_n(location = "level") %>%  -->
<!--   modify_spanning_header(everything() ~ "**OS**") %>% as_gt() -->

<!-- # glm(os_event ~ ivig_cat + cytogenetics + high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1, -->
<!-- #     data = bsab_data, -->
<!-- #     family = "binomial") %>% -->
<!-- #   tbl_regression(exponentiate = TRUE,  -->
<!-- #                  intercept = TRUE) %>% -->
...
<!-- coxph(Surv(time = bsab_data$pfs_time_from_initiation_months,  -->
<!--                    event = bsab_data$pfs_event) ~ ivig_cat + cytogenetics + high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1,  -->
<!--               data = bsab_data)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
<!--   add_nevent(location = "level") %>% add_n(location = "level") %>%  -->
<!--   modify_spanning_header(everything() ~ "**PFS**") %>% as_gt() -->

<!-- # glm(pfs_event ~ ivig_cat + cytogenetics + high_marrow_burden_50_percent_p_cs_if_checked_within_30_days_before_bs_ab_initiation_n_0_y_1, -->
<!-- #     data = bsab_data, -->
<!-- #     family = "binomial") %>% -->
<!-- #   tbl_regression(exponentiate = TRUE,  -->
<!-- #                  intercept = TRUE) %>% -->
...
# Duration of BsAb therapy

KM medians
```{r}
survfit(Surv(duration_of_tec_days, tec_event) ~ 1, data = bsab_data)
survfit(Surv(duration_of_tec_days, tec_event) ~ ivig_cat, data = bsab_data)
```


```{r KM duration_of_tec_days}
ggsurvplot(survfit(Surv(duration_of_tec_days, tec_event) ~ 1, 
                   data = bsab_data),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
...
ggsurvplot(survfit(Surv(duration_of_tec_days, tec_event) ~ ivig_cat, 
                   data = bsab_data),
           # title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(15, "bold", "black"),
